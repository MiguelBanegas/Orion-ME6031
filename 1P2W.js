...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function exportToPDF() {
    try {
      ...
      const connectionType = "1 FASE 2 CONDUCTORES";
      const serialNumber =
      ...
      try {
        logoDataUrl = await loadImageDataUrl(
          "flaticon/ingenieria-electrica.png",
        );
      } catch (error) {
      ...
      const summaryRows = [
        {
          label: "Voltaje (V)",
          stats: getStats(reportData.map((item) => item.UA)),
        },
        {
          label: "Corriente (A)",
          stats: getStats(reportData.map((item) => item.IA)),
        },
        {
          label: "Potencia Activa (kW)",
          stats: getStats(reportData.map((item) => item.PA)),
        },
        {
          label: "Factor de Potencia",
          stats: getStats(reportData.map((item) => item.PFA)),
        },
        {
          label: "THD Tensión (%)",
          stats: getStats(reportData.map((item) => item.UTHA)),
        },
        {
          label: "THD Corriente (%)",
          stats: getStats(reportData.map((item) => item.ITHA)),
        },
      ];
