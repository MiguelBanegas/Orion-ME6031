...
const API_URL =
  "https://api.orion.mabcontrol.ar/api/energy/3p4w?device_uid=ESP32_001";
...
document.getElementById("csvFile").addEventListener("change", function (e) {
  ...
  if (!isValidExtension || !isValidSuffix) {
    alert(
      'Por favor, selecciona un archivo que contenga "3P4W" en su nombre y termine en .csv.',
    );
    return;
  }
...
if (aplicarFiltro) {
  aplicarFiltro.addEventListener("click", async function () {
    ...
    try {
        ...
        setTimeout(() => {
          try {
            ...
            datosActivos = datos.filter((row) => {
              ...
              // Para 3P4W usamos UAvg, IAvg y PSum como referencia principal de filtrado
      ...
      const url = `https://api.orion.mabcontrol.ar/api/energy/3p4w/filter?${params}`;
...
function crearGraficos(datos) {
  ...
  const labels = downsampleArray(
    datos.map((row) => row.Time.substring(0, 5)),
    MAX_POINTS,
  );
  ...
  if (ctxVoltaje) {
    ...
    charts.graficaVoltaje = new Chart(ctxVoltaje, {
      ...
      options: {
        ...
        plugins: { title: { display: true, text: "Tensión L-N (V)" } },
        scales: {
  ...
  if (ctxCorriente) {
    ...
    charts.graficaCorriente = new Chart(ctxCorriente, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: { title: { display: true, text: "Corrientes (A)" } },
        scales: {
          x: { ticks: { maxRotation: 45, minRotation: 45 } },
          y: {
    ...
    const paDatasets = [
      {
        label: "PA",
        data: downsampleArray(
          datos.map((row) => (row.PA !== undefined ? row.PA / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.A,
        tension: 0.1,
      ...
      },
      {
        label: "PB",
        data: downsampleArray(
          datos.map((row) => (row.PB !== undefined ? row.PB / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.B,
        tension: 0.1,
      ...
      },
      {
        label: "PC",
        data: downsampleArray(
          datos.map((row) => (row.PC !== undefined ? row.PC / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.C,
        tension: 0.1,
      ...
      },
      {
        label: "PSum",
        data: downsampleArray(
          datos.map((row) => (row.PSum !== undefined ? row.PSum / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.Sum,
        tension: 0.1,
  ...
  if (ctxPR) {
    const prDatasets = [
      {
        label: "QA",
        data: downsampleArray(
          datos.map((row) => (row.QA !== undefined ? row.QA / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.A,
        tension: 0.1,
      ...
      },
      {
        label: "QB",
        data: downsampleArray(
          datos.map((row) => (row.QB !== undefined ? row.QB / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.B,
        tension: 0.1,
      ...
      },
      {
        label: "QC",
        data: downsampleArray(
          datos.map((row) => (row.QC !== undefined ? row.QC / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.C,
        tension: 0.1,
      ...
      },
      {
        label: "QSum",
        data: downsampleArray(
          datos.map((row) => (row.QSum !== undefined ? row.QSum / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.Sum,
        tension: 0.1,
    ...
    charts.graficaPotenciaReactiva = new Chart(ctxPR, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: { title: { display: true, text: "Potencia Reactiva (kVar)" } },
        scales: {
          x: { ticks: { maxRotation: 45, minRotation: 45 } },
          y: {
            min: prRange.min,
  ...
  if (ctxPS) {
    const psDatasets = createDatasets(datos, "S").map((ds) => ({
      ...ds,
      data: ds.data.map((v) => v / 1000),
    }));
    ...
    charts.graficaPotenciaAparente = new Chart(ctxPS, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: { title: { display: true, text: "Potencia Aparente (kVA)" } },
        scales: {
          x: { ticks: { maxRotation: 45, minRotation: 45 } },
          y: {
            min: psRange.min,
  ...
  if (ctxEP) {
    const epDatasets = [
      {
        label: "EPA",
        data: downsampleArray(
          datos.map((row) => (row.EPA !== undefined ? row.EPA / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.A,
        tension: 0.1,
      ...
      },
      {
        label: "EPB",
        data: downsampleArray(
          datos.map((row) => (row.EPB !== undefined ? row.EPB / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.B,
        tension: 0.1,
      ...
      },
      {
        label: "EPC",
        data: downsampleArray(
          datos.map((row) => (row.EPC !== undefined ? row.EPC / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.C,
        tension: 0.1,
      ...
      },
      {
        label: "EPSum",
        data: downsampleArray(
          datos.map((row) => (row.EPSum !== undefined ? row.EPSum / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.Sum,
        tension: 0.1,
    ...
    charts.graficaEnergiaActiva = new Chart(ctxEP, {
      type: "line",
      data: {
        labels: labels,
        datasets: epDatasets,
  ...
  if (ctxEQ) {
    const eqDatasets = [
      {
        label: "EQA",
        data: downsampleArray(
          datos.map((row) => (row.EQA !== undefined ? row.EQA / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.A,
        tension: 0.1,
      ...
      },
      {
        label: "EQB",
        data: downsampleArray(
          datos.map((row) => (row.EQB !== undefined ? row.EQB / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.B,
        tension: 0.1,
      ...
      },
      {
        label: "EQC",
        data: downsampleArray(
          datos.map((row) => (row.EQC !== undefined ? row.EQC / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.C,
        tension: 0.1,
      ...
      },
      {
        label: "EQSum",
        data: downsampleArray(
          datos.map((row) => (row.EQSum !== undefined ? row.EQSum / 1000 : 0)),
          MAX_POINTS,
        ),
        borderColor: COLORS.Sum,
        tension: 0.1,
    ...
    charts.graficaEnergiaReactiva = new Chart(ctxEQ, {
      type: "line",
      data: {
        labels: labels,
        datasets: eqDatasets,
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: { title: { display: true, text: "Energía Reactiva (kVarh)" } },
        scales: {
          x: { ticks: { maxRotation: 45, minRotation: 45 } },
          y: {
            min: eqRange.min,
...
function crearHistogramasComplementarios(datos) {
  console.log("Iniciando creación de histogramas complementarios (3P4W)");

  // 1. Histograma de Voltaje (UA, UB, UC, 0 decimales)
  ...
  // 2. Histograma de Corriente (IA, IB, IC, 0 decimales)
  ...
  // 4. Histograma de Factor de Potencia (PF o PFAvg, 2 decimales)
  const pfData = datos
    .map((row) => parseFloat(row.PF || row.PFAvg))
    .filter((v) => !isNaN(v));
  crearHistograma(
    "histoPF",
...
function crearHistograma(canvasId, datasets, label, unit, precision = 0) {
  ...
  if (typeof charts !== "undefined" && charts[canvasId]) {
...
async function exportToPDF() {
  try {
    ...
    const connectionType = "3 FASES 4 CONDUCTORES";
    const serialNumber =
    ...
    let yPos = drawHeader(true); // Posición inicial Y después de los títulos
    ...
    for (let i = 0; i < graphConfigs.length; i++) {
      ...
      if (canvas) {
        ...
        if (yPos + graphHeight + titleHeight > pageHeight - footerSpace) {
          doc.addPage();
          yPos = drawHeader(false); // Reiniciar la posición Y
        }
  ...
  return apiData.map((item) => {
    try {
      ...
      return {
        ...
        UB: parseFloat(item.ub) || 0,
        UC: parseFloat(item.uc) || 0,
        ...
        UTHB: parseFloat(item.uthb) || 0,
        UTHC: parseFloat(item.uthc) || 0,
